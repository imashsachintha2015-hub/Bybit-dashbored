...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Always-on HTF Swing Discovery layer. This is deliberately separate from
  // the selected execution mode: SCALP/SURESHOT/SWING changes what may trade,
  // but must never turn off discovery of larger 2.2%-6.5% opportunities.
  const swingObservers = {}, latestSwingStates = {}, swingOpportunityState = {};
  const SWING_DISCOVERY_MIN_PCT = 2.2;
  const SWING_DISCOVERY_MAX_PCT = 6.5;
  const SWING_DISCOVERY_INTERVAL_MS = 30000;
  let lastSwingDiscoveryAt = 0;
  ...
  WATCHLIST.forEach(sym => {
    ...
    // Separate standard/HTF observer. It never calls executeEntry(), never
    // consults the live execution supervisor, and never places an order.
    // Its only job is to continuously discover larger swing opportunities
    // while the selected execution mode can remain scalp or sureshot.
    swingObservers[sym] = new MasisEngine({
      symbol: sym,
      metaLearner: new SwarmMetaLearner.MetaLearner(),
      swarmMode: 'observe'
    });
  ...
  // Supervisor/execution handoff guard. A verdict is valid only for the exact
  // candidate that was reviewed. This prevents an async supervisor response
  // for e.g. LINK SHORT from authorising a later LINK LONG state.
  const supervisorPending = new Map();
  const supervisorCompleted = new Map();
  const supervisorLogged = new Set();

  function executionCandidateKey(sym, s) {
    const targets = Array.isArray(s.takeProfit) ? s.takeProfit : [];
    return [
      sym, s.setupType || '', s.direction || '',
      Number(s.entry || 0).toFixed(8),
      Number(s.stopLoss || 0).toFixed(8),
      targets.map(v => Number(v || 0).toFixed(8)).join(','),
      currentStrategyMode || 'standard'
    ].join('|');
  }
  ...
  async function maybeConsultSupervisor(sym, s) {
    ...
    const completed = supervisorCompleted.get(candidateKey);
    if (completed) return completed;

    const pending = supervisorPending.get(candidateKey);
    if (pending) return null;

    const gate = llmGovernor.shouldConsult(candidate, ctx);
    if (!gate.allowed) {
      if (gate.cached) {
        const cached = Object.assign({}, gate.cached, {
    ...
    const promise = (async () => {
      try {
        ...
        const verdict = Object.assign({}, res, {
          symbol: sym, direction: s.direction, at: Date.now(), candidateKey
        });
        ...
        supervisorPending.delete(candidateKey);
      }
    })();

    supervisorPending.set(candidateKey, promise);
    return null;
  }
  ...
  async function managePositions() {
    ...
    for (const pos of openPositionsSnapshot) {
      ...
      // HTF Swing Runner time discipline: do not leave a flat consolidation
      // position open indefinitely. Only scratch genuinely flat swing trades
      // (roughly +/-0.25R) after 3.5h; losing/winning positions outside that
      // band continue through the normal structural position manager.
      const tradeAgeMs = Date.now() - Number(trade.openedAt || Date.now());
      const tradeIsScalp = !!trade.isScalp || (trade.setupName && trade.setupName.startsWith('SCALP_'));
      const preEvalR = positionManager.currentR(trade, mark);
      const flatSwingScratch = !tradeIsScalp &&
        tradeAgeMs >= 3.5 * 60 * 60 * 1000 &&
        Math.abs(preEvalR) <= 0.25;

      if (flatSwingScratch) {
        const detail = `HTF swing flat-consolidation scratch after ${(tradeAgeMs / 3600000).toFixed(1)}h at ${preEvalR.toFixed(2)}R`;
        logEvent(`${pos.symbol} ${detail}`);
        if (autoTradingArmed) {
          await closePositionSlice(pos, trade, parseFloat(pos.size), 'SWING_TIME_SCRATCH', detail);
          continue;
        }
      }
      ...
      for (const iv of ANALYSIS_INTERVALS) {
        try {
          ...
          if (candles && candles.length) {
            engines[sym].seedCandles(INTERVAL_TO_TF[iv], candles);
            if (swingObservers[sym]) swingObservers[sym].seedCandles(INTERVAL_TO_TF[iv], candles);
          }
        } catch (e) {
  ...
  wsClient.on('ticker', (d) => {
    ...
    if (swingObservers[sym]) swingObservers[sym].processTicker(d.data);
  ...
  wsClient.on('orderbook', d => {
    ...
    if (swingObservers[d.symbol]) swingObservers[d.symbol].processOrderBook(d.data);
  });
  ...
  wsClient.on('kline', (d) => {
    ...
    const swingEng = swingObservers[d.symbol];
    if (swingEng && tf) d.data.forEach(k => swingEng.processKline(tf, k));
  ...
  wsClient.on('trade', (d) => {
    ...
    if (swingObservers[sym]) swingObservers[sym].processTrades(trades);
  ...
  // ─── Always-on HTF Swing Discovery (OBSERVE ONLY) ───────────────────
  //
  // This scanner is intentionally independent of currentStrategyMode.
  // SCALP/SURESHOT can be the active execution mode while this layer keeps
  // watching for larger HTF opportunities. Discovery is informational only:
  // it never arms trading, bypasses risk, calls executeEntry(), or places an
  // order. A swing opportunity becomes executable only when SWING mode is
  // explicitly selected and the normal candidate/supervisor/risk pipeline
  // accepts it.
  function discoverHtfSwingOpportunities(now = Date.now()) {
    if (now - lastSwingDiscoveryAt < SWING_DISCOVERY_INTERVAL_MS) return;
    lastSwingDiscoveryAt = now;

    const found = [];
    for (const sym of WATCHLIST) {
      const eng = swingObservers[sym];
      if (!eng) continue;

      const s = eng.getState();
      latestSwingStates[sym] = s;
      if (!s || !s.entry || !Array.isArray(s.takeProfit) || !s.takeProfit.length) continue;

      const entry = Number(s.entry);
      const direction = String(s.direction || '').toUpperCase();
      const targets = s.takeProfit.map(Number).filter(Number.isFinite);
      if (!Number.isFinite(entry) || entry <= 0 || !targets.length) continue;

      const target = direction === 'SHORT'
        ? Math.min(...targets.filter(t => t < entry))
        : Math.max(...targets.filter(t => t > entry));

      if (!Number.isFinite(target)) continue;

      const movePct = Math.abs((target - entry) / entry) * 100;
      if (movePct < SWING_DISCOVERY_MIN_PCT || movePct > SWING_DISCOVERY_MAX_PCT) continue;

      const key = [
        sym, direction, s.setupType || '',
        entry.toFixed(6), target.toFixed(6)
      ].join('|');

      const previous = swingOpportunityState[sym];
      const changed = !previous || previous.key !== key;
      swingOpportunityState[sym] = {
        key,
        symbol: sym,
        direction,
        setupType: s.setupType || 'HTF_SWING',
        entry,
        target,
        movePct,
        grade: s.grade || '',
        ...
        regime: s.regime || '',
        bias: s.bias || '',
        detectedAt: previous && !changed ? previous.detectedAt : now,
        modeAtDetection: currentStrategyMode
      };

      found.push(swingOpportunityState[sym]);

      if (changed) {
        logEvent(
          `HTF SWING DISCOVERY ${sym} ${direction}: +${movePct.toFixed(2)}% target ` +
          `(entry ${entry.toFixed(6)} → target ${target.toFixed(6)}) | ` +
          `grade ${s.grade || '--'} score ${s.score || '--'} | ` +
          `execution mode=${currentStrategyMode.toUpperCase()} — OBSERVE ONLY`
        );
      }
    }

    // Keep the global discovery state small and current.
    for (const sym of WATCHLIST) {
      if (!found.some(x => x.symbol === sym) && swingOpportunityState[sym]) {
        const age = now - swingOpportunityState[sym].detectedAt;
        if (age > 6 * 60 * 60 * 1000) delete swingOpportunityState[sym];
      }
    }
  }
  ...
  setInterval(async () => {
    ...
    for (const sym of WATCHLIST) {
      ...
      // HTF Swing discovery is always active, regardless of SCALP/SURESHOT/
      // SWING execution mode. It does not execute anything.
      discoverHtfSwingOpportunities();

      if (s.grade && s.grade !== 'C' && s.grade !== 'D') {
        // Start/refresh the supervisor review first. It is deliberately not
        // awaited here because 30 symbols must not serialize behind model calls.
        // executeEntry() will only proceed on a completed verdict for this exact
        // candidate on a subsequent tick.
        maybeConsultSupervisor(sym, s);
      }
  ...
  if (stdBtn) stdBtn.addEventListener('click', () => {
    ...
    logEvent('Strategy profile switched to SWING MODE (15m–1h structural targets) — HTF discovery remains always-on in all modes');
