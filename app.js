...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // HTF Swing Runner: independent candidate engine. It remains live in every
  // dashboard mode, but its candidates still pass the normal supervisor/risk
  // execution gates before an order can be submitted.
  const htfSwingEngines = {}, latestHtfSwingStates = {}, htfSwingSeen = new Map();
  const HTF_SWING_MIN_TARGET_PCT = 2.2;
  const HTF_SWING_MAX_TARGET_PCT = 6.5;
  const HTF_SWING_SCAN_MS = 30000;
  let lastHtfSwingScanAt = 0;
  ...
  WATCHLIST.forEach(sym => {
    ...
    // Separate stateful engine for the continuously-running HTF swing strategy.
    // It shares the market stream but does not share mutable engine state with
    // the selected SCALP/SURESHOT/SWING profile.
    htfSwingEngines[sym] = new MasisEngine({ symbol: sym, metaLearner, swarmMode: 'observe' });
      ...
      for (const iv of ANALYSIS_INTERVALS) {
        try {
          ...
          if (candles && candles.length) {
            ...
            engines[sym].seedCandles(tf, candles);
            if (htfSwingEngines[sym]) htfSwingEngines[sym].seedCandles(tf, candles);
          }
        } catch (e) {
  ...
  wsClient.on('ticker', (d) => {
    ...
    if (htfSwingEngines[sym]) htfSwingEngines[sym].processTicker(d.data);
  ...
  wsClient.on('orderbook', d => {
    ...
    if (htfSwingEngines[d.symbol]) htfSwingEngines[d.symbol].processOrderBook(d.data);
  });
  ...
  wsClient.on('kline', (d) => {
    ...
    const htfEng = htfSwingEngines[d.symbol];
    if (htfEng && tf) d.data.forEach(k => htfEng.processKline(tf, k));
  ...
  wsClient.on('trade', (d) => {
    ...
    if (htfSwingEngines[sym]) htfSwingEngines[sym].processTrades(trades);
  ...
  // ─── Always-on HTF Swing Runner ──────────────────────────────────────
  // Discovery is independent of the selected UI mode. Execution is NOT a
  // direct exchange call: the candidate is sent through the existing
  // supervisor -> exact-candidate verdict -> risk -> executeEntry pipeline.
  function scanHtfSwingRunner(now = Date.now()) {
    if (now - lastHtfSwingScanAt < HTF_SWING_SCAN_MS) return;
    lastHtfSwingScanAt = now;

    for (const sym of WATCHLIST) {
      const eng = htfSwingEngines[sym];
      if (!eng) continue;

      const s = eng.getState();
      latestHtfSwingStates[sym] = s;
      ...
      if (!Number.isFinite(entry) || entry <= 0 || (dir !== 'BUY' && dir !== 'SELL' && dir !== 'LONG' && dir !== 'SHORT')) continue;
      const direction = (dir === 'BUY' || dir === 'LONG') ? 'LONG' : 'SHORT';
      const tps = s.takeProfit.map(Number).filter(Number.isFinite);
      const usable = direction === 'LONG' ? tps.filter(x => x > entry) : tps.filter(x => x < entry);
      if (!usable.length) continue;
      const target = direction === 'LONG' ? Math.max(...usable) : Math.min(...usable);
      const movePct = Math.abs((target - entry) / entry) * 100;
      if (movePct < HTF_SWING_MIN_TARGET_PCT || movePct > HTF_SWING_MAX_TARGET_PCT) continue;
      ...
      const candidate = { ...s,
        ...
        entry,
        takeProfit: usable,
        htfTargetPct: movePct,
        discoveredAt: now
      };
      const key = executionCandidateKey(sym, candidate);
      if (htfSwingSeen.get(sym) === key) continue;
      htfSwingSeen.set(sym, key);
      ...
      if (candidate.grade && candidate.grade !== 'C' && candidate.grade !== 'D' && candidate.stopLoss) {
        ...
        setTimeout(() => executeEntry(sym, candidate), 250);
      }
    }
  }
