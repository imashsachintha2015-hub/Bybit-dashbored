...
This persists just the arm flag and the configured risk-per-trade percent
through kv.py, so every device shows the same, current state. It does NOT
make it safe to have two tabs both actively armed at once: each tab's
engine runs and places orders independently, with no shared lock between
them, so two tabs seeing the same signal in the same cycle can both fire
an order. Treat this as "which device is currently driving trades" staying
visible and consistent across devices, not as multi-device coordination.
"""
from .kv import kv_get_json, kv_set_json

STATE_KEY = "auto_trade_state"

DEFAULT_STATE = {"armed": False, "riskPerTradePct": 0.5}
...
def save(armed, risk_per_trade_pct):
    state = {
        "armed": bool(armed),
