...
The repository keeps app.js large and actively edited. Railway applies
it to the freshly copied source during Docker build.
"""
...
def safe_patch(txt, old_snippet, new_snippet, name):
    if old_snippet in txt:
        txt = txt.replace(old_snippet, new_snippet, 1)
        print(f"[HTF TRACE] {name}: patched")
    else:
        print(f"[HTF TRACE] {name}: anchor not found (skipped non-fatal)")
    return txt
...
text = safe_patch(text, anchor, helper + anchor, "supervisor function helper")

# 2) Mark the candidate's supervisor routing decision
...
text = safe_patch(text, old, new, "HTF candidate routing")
...
text = safe_patch(text, old, new, "HTF candidate else")
...
text = safe_patch(text, old, new, "supervisor start")
...
text = safe_patch(text, old, new, "cached supervisor")
...
text = safe_patch(text, old, new, "supervisor final logging")
...
text = safe_patch(text, old, new, "supervisor error")
...
text = safe_patch(text, old, new, "risk gate")
...
text = safe_patch(text, old, new, "risk sizing")
...
text = safe_patch(text, old, new, "execution success")
...
text = safe_patch(text, old, new, "supervisor wait")
...
text = safe_patch(text, old, new, "supervisor veto")

APP.write_text(text, encoding="utf-8")
print("[HTF TRACE] app.js processing complete")
